...
def imprumut():
    ...
    window.geometry('%dx%d' % (ws, hs))
    window.iconbitmap('carti_i.ico')
    coloane=[0,1,2,3,4,5,6,7]
    tabel=ttk.Treeview(window,columns=coloane,show="headings",height=26)
    tabel.pack()

    adus=Button(window,text="Returnată",bg="#547F5D",fg="white",font=("Helvetica",20),width=20,command=returnata)
    adus.pack()

    msj_elev=Button(window,text="Notifică elevii",bg="#547F5D",fg="white",font=("Helvetica",20),width=20,command=mesaj)
    msj_elev.pack()
...
def search():
    #funcția care actualizează filtrul
    def schimbare(event):  
        global criteriu
        criteriu=filtru.get()
        print(criteriu)
    ...
    #funcția care șterge toate rândurile din tabel
    def stergere_elemente_arbore():  
        x = tree.get_children()
        for item in x:
            tree.delete(item)

    #functia care face cautarea in baza de date în funcție de bara de căutare
    def cauta(event):      
        cuvant=caseta.get()
        print(cuvant)
        if criteriu=='Cod':
    ...
    def inserare():
        ...
        canvas=Canvas(geam,width=ws,height=hs,background="#c7d3d4")
    ...
    #buton pentru generare QR
    genqr=Button(frame3,width=20,text="Generează cod QR",command=generareQR,font=("Helvetica",14),bg="#A085C2",fg="black") 
    genqr.pack(side=RIGHT)
    #buton pentru ștergere carte
    sterge=Button(frame3,width=15,text="Șterge carte",command=stergere,font=("Helvetica",14),bg="#A085C2",fg="black") 
    sterge.pack(side=RIGHT)
    #buton pentru inserare carte
    adauga=Button(frame3,width=15,text="Adaugă carte",command=inserare,font=("Helvetica",14),bg="#A085C2",fg="black") 
    adauga.pack(side=RIGHT)

    #filtru (caută după)
    filtru=ttk.Combobox(frame3,height=30,font=("Helvetica",16))  
    ...
    #bara de căutare
    frame_caut=Frame(window)
    frame_caut.pack(anchor=CENTER)
    label_ce=Label(frame_caut,text="Ce caut:",font=("Helvetica",20))
    label_ce.pack(side=LEFT)
    caseta=Entry(frame_caut,font=("Helvetica",20),fg="black")
    caseta.pack(side=LEFT)
    
    global tree
    coloane=(1,2,3,4,5,6,7)
    tree=ttk.Treeview(window,columns=coloane,height=65) #tabelul cu afișări
    ...
    for rand in date:
        ...
        nr=cursor.fetchall()
        nrc=nr[0]
        ID=tree.insert("",END,text=str(id),values=[rand[1],rand[2],rand[3],rand[4],rand[5],nrc])
